//  main.swift
//  Y16J (Implmenting 'Advanced Gear Shift' Logic)
...
let THREADPOOL_COUNT = 64 // 128
let MACRO_BLOCK_SIZE = 1*8*1024

print("Hello, World!")

print("Init Random Data.......")
let bigNum  = BigUInt.randomInteger(withExactWidth: THREADPOOL_COUNT*MACRO_BLOCK_SIZE*8)
let bigData = bigNum.serialize().toArray(type: UInt8.self).chunked(into: 512)
...
DispatchQueue.concurrentPerform(iterations: THREADPOOL_COUNT, execute: { indexGCD in
    ...
    if (indexGCD < bigData.count) {
        ...
        print(compSZ.length-32, "< SZ (-32 bytes for fixed LZMA header)", "count:", bulkStatsZ.count)
    }
})
